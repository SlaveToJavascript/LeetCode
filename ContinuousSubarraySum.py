...
# ✅ ALGORITHM 2: HASHMAP
# Starting from nums[0], get the sum of each subarray from nums[0] to nums[i] and divide that by k
...
# NOTE: initialize hashmap with {0: -1} so that if the first value is a multiple of k, we don't return True immediately

# TIME COMPLEXITY: O(n)
# SPACE COMPLEXITY: O(n)

def checkSubarraySum(self, nums, k):
    if len(nums) < 2: return False # since length of good subarray >= 2
    ...
    for idx, num in enumerate(nums):
        ...
        if rem in hm: # that means we found a subarray that is divisible by k
            if idx-hm[rem] >= 2: # size of good subarray must be at least 2
                return True
        else: hm[rem] = idx # add {remainder:closing_index} key-value pair to hm
    return False