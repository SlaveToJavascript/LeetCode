...
# bridges is an array of coordinates of the bridge cells built in order, where bridges[i] is the coordinates of the bridge cell built on the i'th day
# Given bridges, return the minimum no. of days required to build a complete bridge (made up of bridge cells) across the river, where we can get from the left side to the right side
...
# The bridge cells are marked B in the matrix below
    # The capital B's are the bridge cells that form the actual bridge -> there are 7 B's, meaning 7 days are taken to build the complete bridge
# |   b|
# |BB  |
# | B  |
# | BB |
...
# TIME COMPLEXITY: O(m*n)
    # m and n = no. of rows and cols
    # worst case: every cell is a bridge cell -> each cell is processed once -> no. of cells to process = m*n
# SPACE COMPLEXITY: O(B)
    # B = no. of bridge cells
