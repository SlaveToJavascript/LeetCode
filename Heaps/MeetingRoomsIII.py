...
def mostBooked(n, meetings):
    ...
    for start, end in meetings: # start and end time of current meeting
        while busyHeap and busyHeap[0][0] <= start: # check for any available rooms by checking if end time(s) of the previous meeting(s) (if any) is before the start time of current meeting
            e, r = heapq.heappop(busyHeap)
            heapq.heappush(availHeap, r) # pop these meeting(s) from busyHeap and push their room(s) to availHeap
        
        if availHeap: # if availHeap is not empty (i.e. there are rooms available),
            room = heapq.heappop(availHeap) # available room with smallest room no.
            heapq.heappush(busyHeap, [end, room]) # push room no. to busyHeap (this is the room assigned to current meeting) together with current meeting's end time
        else: # if there are no more empty rooms for the current meeting
            prev_end_time, room = heapq.heappop(busyHeap) # end time of the meeting that ends the earliest will be start time of our current meeting; room for that meeting will be current meeting's room
            heapq.heappush(busyHeap, (prev_end_time + end-start, room)) # the revised start time of current delayed meeting = end time of previous meeting that ended + duration of current meeting
